...
public class Eclipse extends AShape{
	...
	public Eclipse(int x, int y, Color color,double  width,  double height, Stroke stroke, boolean filled) {
	...
	height = Math.abs(secondPoint.y-location.y);
		...
		g.setStroke(stroke);
		if (getFilled()) {
			g.fillOval(getx, gety,(int) width ,(int)height);
		}
		else g.drawOval(getx, gety,(int) width ,(int)height);
	}
